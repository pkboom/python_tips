...
R = len(G)
C = max(len(x) for x in G)
for r in range(R):
    while len(G[r]) < C:
        G[r] += " "
    assert len(G[r]) == C

if infile == "example.in":
    CUBE = C // 4
    REGION = [(0, 2), (1, 0), (1, 1), (1, 2), (3, 2), (3, 3)]
else:
    CUBE = C // 3
    ...
    while i < len(instr):
        ...
        for _ in range(n):
            ...
            if G[rr][cc] == " ":
                ...
                if part == 2:
                    print(
                        f"r={r} c={c} rr={rr} cc={cc} nr={nr} nc={nc} region={getRegion(r,c)} d={d} newRegion={getRegion(nr,nc)} nd={nd}"
                    )
                if G[nr][nc] == "#":
                    break
                (r, c, d) = (nr, nc, nd)
